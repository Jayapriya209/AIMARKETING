...
def top_5_selection(analysed_tweets, engagement_type: str):
    ...
    return filtered_df.nlargest(5, columns=['engagement_score']).values.tolist() #nlargest means n number of data
...
#system prompt means user prompt+ context+ instruction
        system_prompt = f"""
        ...
        out_a = execute_gemini_for_tweet_creation( #niche lLM
            ...
            model="gemini-2.5-flash-lite" # prompt + model will give niche LLM
        )
        out_dict = json.loads(out_a)
        tweet_a = out_dict['tweet']
        print("TWEET_A ======>")
        print(tweet_a)
        sleep(2) # asking LLM to take rest before tweet b
        ...
        sleep(2) # we r calling gemini api so to avoid like they r blocking bcoz using again again
